...
def fun8():
    ...
    def fun2():
        ...
        con.commit()
        if d=='' or b=='' or c=='':
             ttk.tkMessageBox.showerror("Oops","You can't Enter the leave any field empty")
        else:     
             if b=="economic":
                cur.execute("delete from economic2 where adno=(?) and boarding=(?)",(d,b,))
                cur.execute("select * from economic2")
                ttk.tkMessageBox.showinfo("your reservation is cancelled",cur.fetchall())
             else:
...
def fun9():
    ...
    def fun10():
        ...
        cur=con.cursor()
        if a=='' or b=='' or c=='':
            ttk.tkMessageBox.showerror("Error","Cant leave any field empty")
             ...
             if a!=b:
                 ...
                 con.commit()
                 e=cur.fetchall()
                 ttk.tkMessageBox.showinfo("flights availble are",e)
             else:
                ttk.tkMessageBox.showerror("Oops","boarding and destination can't me same")
...
#e3.grid(row=3,column=1)
    ...
    def fun():
        ...
        if a=='' or b=='' or c=='' or d=='' or f=='' or g=='':
            ttk.tkMessageBox.showerror("OOPS","you can't leave any field empty")
        else :
            if d=='Economic':
                ...
                if a!=b:
                    ...
                    ttk.tkMessageBox.showinfo("congrats","your seat has been reserved")
                    con.commit()
                    cur.execute("select * from economic2 where adno=(?)",(c,))
                    ttk.tkMessageBox.showinfo("records",cur.fetchall())
                else:
                    ttk.tkMessageBox.showerror("Error","you can't choose same city")
            if d=='BusinessClass':
